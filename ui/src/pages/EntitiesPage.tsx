...
export function EntitiesPage() {
  ...
  async function handleSave() {
    ...
    try {
      const updated = await api.saveEntityYaml(selectedName, editorContent)
      const yaml = updated.raw_yaml ?? editorContent
      setSavedContent(yaml)
      setDetail(updated)
      setEntities(prev => prev.map(e =>
        e.name === selectedName
          ? { ...e, attribute_count: updated.attributes.length, relation_count: updated.relations.length }
          : e
      ))
      setValidStatus('pending')
      api.validateEntity(selectedName)
    ...
    <div className="flex h-full -mx-6 -my-6 overflow-hidden">
      ...
      <aside className="w-56 shrink-0 border-r flex flex-col overflow-hidden bg-card">
        <div className="px-4 py-3 border-b">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{entities.length} defined</p>
        </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
              <EditSection
                ...
                yamlContent={
                  <YamlEditor
                    name={`${selectedName.replace(/(?<=[a-z0-9])([A-Z])/g, '_$1').toLowerCase()}.yaml`}
                    historyKey={{ type: 'entity', name: selectedName }}
