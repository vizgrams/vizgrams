...
function ColumnSelectorPanel({
  attributes, features, selected, onApply,
}: {
  attributes: AttributeOut[]; features: FeatureOut[]; selected: string[]; onApply: (cols: string[]) => void
}) {
      ...
      {open && (
        <div className="px-4 py-3 space-y-3">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on ? 'bg-primary text-primary-foreground border-primary' : 'bg-transparent text-muted-foreground border-border hover:border-foreground/40 hover:text-foreground',
                  )}
                >
                  {item.isFeature && (
                    <span className={cn('text-[9px] font-bold border rounded px-0.5 leading-none', on ? 'border-primary-foreground/50 text-primary-foreground/80' : 'border-primary/30 text-primary/60')}>ƒ</span>
                  )}
                  {item.label}
                </button>
              )
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-border">
            <button
              onClick={() => { const ordered = items.filter((i) => draft.has(i.id)).map((i) => i.id); onApply(ordered); setOpen(false) }}
              className="text-xs px-3 py-1.5 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors"
            >
              Apply
            </button>
            <button onClick={() => { setDraft(new Set(selected)); setOpen(false) }} className="text-xs px-3 py-1.5 border rounded-md hover:bg-muted transition-colors">
              Cancel
            </button>
          </div>
        </div>
...
function SortIcon({ column, sort }: { column: string; sort: SortState | null }) {
  ...
  return sort.direction === 'ASC' ? <ChevronUp className="inline h-3 w-3 ml-1" /> : <ChevronDown className="inline h-3 w-3 ml-1" />
}
...
export function EntityListFrame({
  entity,
  onNavigate,
}: {
  entity: string
  ...
  useEffect(() => {
    setOffset(0); setFilters(''); setPendingFilter(''); setSort(null)
    setSortInitialised(false); setSelectedCols(null)
  }, [entity])

  if (schemaState.status === 'ok' && !sortInitialised) {
    const schema = schemaState.data
    const first = schema.display_order[0]
    setSort(first ? { column: first.column, direction: first.direction.toUpperCase() as SortDir } : null)
    setSortInitialised(true)
    if (selectedCols === null) {
      const pk = schema.attributes.find((a) => a.semantic === 'PRIMARY_KEY')?.name ?? schema.attributes[0]?.name
      const list = schema.display_list.length > 0 ? schema.display_list : schema.attributes.slice(0, 6).map((a) => a.name)
  ...
  const queryState = useApi(() => {
    ...
    return api.executeInline({ entity, detail: true, attributes: attrCols, where, order_by }, PAGE_SIZE, offset)
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          {queryState.status === 'ok' && queryState.data && (
            <Badge className="text-muted-foreground">{queryState.data.total_row_count.toLocaleString()} rows</Badge>
          )}
          <button
            disabled={jobState.phase === 'running'}
            onClick={async () => { resetJob(); const job = await api.runMapper(entity); startJob(job.job_id, 'Run Mapper') }}
          ...
          >
            <Play className={cn('h-3.5 w-3.5', jobState.phase === 'running' && 'animate-pulse')} /> Run Mapper
          </button>
          <button
            disabled={jobState.phase === 'running'}
            onClick={async () => { resetJob(); const job = await api.rematerializeEntity(entity); startJob(job.job_id, 'Rematerialize') }}
            className="flex items-center gap-1.5 border rounded-md px-3 py-1.5 text-sm hover:bg-muted transition-colors disabled:opacity-50"
          >
            <RefreshCw className={cn('h-3.5 w-3.5', jobState.phase === 'running' && 'animate-spin')} /> Rematerialize
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          onKeyDown={(e) => { if (e.key === 'Enter') { setFilters(pendingFilter); setOffset(0) } }}
        />
        <button className="border rounded-md px-3 py-1.5 text-sm hover:bg-muted transition-colors" onClick={() => { setFilters(pendingFilter); setOffset(0) }}>Apply</button>
        {filters && (
          <button className="border rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted transition-colors" onClick={() => { setFilters(''); setPendingFilter(''); setOffset(0) }}>Clear</button>
        )}
      </div>
          ...
          <>
                      ...
                      return (
                        <th key={col} className="text-left px-3 py-2.5 font-medium">
                          <button
                            ...
                            disabled={isFeat}
                            className={cn('flex items-center gap-0.5 transition-colors', isFeat ? 'cursor-default' : 'hover:text-foreground', sort?.column === col ? 'text-foreground' : 'text-muted-foreground')}
                          >
                            {isFeat && <span className="text-[10px] font-semibold text-primary/60 border border-primary/30 rounded px-0.5 leading-none mr-0.5">ƒ</span>}
                      ...
                      <tr key={i} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                        {displayCols.map((col) => {
                          if (featureIds.has(col)) {
                            ...
                            return <td key={col} className="px-3 py-2 text-muted-foreground">{fv != null ? String(fv) : <span className="opacity-30">—</span>}</td>
                          }
                          ...
                          return (
                            <td key={col} className="px-3 py-2">
                              {col === pk && id
                                ? (
                                  <button
                                    onClick={() => onNavigate({ kind: 'entity-detail', entity, id })}
            ...
            <div className="mt-3 flex items-center justify-between text-sm text-muted-foreground">
              ...
              <div className="flex gap-2">
                <button disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - PAGE_SIZE))} className="border rounded px-2.5 py-1 disabled:opacity-40 hover:bg-muted transition-colors">← Prev</button>
                <button disabled={!result.truncated} onClick={() => setOffset(offset + PAGE_SIZE)} className="border rounded px-2.5 py-1 disabled:opacity-40 hover:bg-muted transition-colors">Next →</button>
              </div>
            </div>
          </>
        )
